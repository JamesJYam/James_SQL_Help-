...
/*This query pulls loan maintenance volume by product and tier for loans.*/


Use Finance


declare @start date = '8-1-2023'
       ,@end date = '7-31-2024'
	   ,@jhdate date = '7-31-2024'
       ,@prodline int = 4			--<-- 2 = Commercial Loans, 4 = QuickStep Loans, 5 = Consumer Loans
...
into #OrigLoans
from 
       (select DataCount,DataDate,Account,OrigAmount,CostDriver from [STP].[CostDriver_2110]
	   where [Status] in (1,4)
	   union all 
	   select DataCount,DataDate,Account,OrigAmount,CostDriver from [STP].[CostDriver_2120]
	   where [Status] in (1,4)
       ) a
...
into #Tiers10
from #Tiers9 t
left join [STP].[AAA_LoanTiers] tt
on t.[Tier #] = tt.[Tier #]
...
select distinct 
	t.*
	,case
		when t.CostDriver between 1000 and 1099 then CONCAT('Originate a New Loan ',t.[Tier Desc])
		when t.CostDriver between 1100 and 1199 then CONCAT('Renew a Loan ',t.[Tier Desc])
